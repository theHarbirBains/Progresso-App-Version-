...
- **You and Progress (Phase 5, now widgets)** -- the app's two mode-agnostic tabs (§11), both since converted from flat sections to the same `AppCard`-per-widget language every other tab uses; Settings stays flat rows, deliberately (see below).
  - _You_ (`screens/ProfileScreen.tsx`, the You tab's root -- a fitness profile, not a social one: no followers/likes/photo grid): the shared header (title, Settings on the right, no hamburger -- §11), a hero identity card (avatar, name/username, `topAccent` in the Workout accent, an outlined Edit Profile), a stats card (Workouts / Total Sets / Total Volume as `StatBlock`s), then the Workouts / Stats / PRs tabs (`CategoryTabs`, outside any card) with the active tab's content in its own widget: Workouts as `ListRow`s plus View All; Stats as a `StatTile` grid, three `WeeklyTrendChart`s and a muscle-group bar list; PRs as `PRsSection`.
