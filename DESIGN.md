...
4. **No Workout/Nutrition toggle.** Moving between Train and Nutrition content is exclusively the bottom nav's job now (point 1) — there is no in-page segmented switch anywhere (the former `ModeToggle` component and its center-"+"-driven `QuickActionMenu` are both gone). `App.tsx`'s `sharedMode` flag — what the mode-agnostic roots (Feed, Progress, You) and `AppSideMenu`'s default fall back to when a screen has no mode identity of its own (see `isNutritionRoute`/`MODE_AGNOSTIC_ROUTES`) — updates itself automatically: landing on any route with an unambiguous mode sets it, so the fallback always reflects wherever the user was a moment ago, with nothing to explicitly report.
