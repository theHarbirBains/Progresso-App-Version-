...
- **`topAccent`** — an optional slim (3px) colored band across the card's top edge, clipped to the card's own corners. The one Strava-style flourish this system borrows (see §17's Feed section): a quick, glanceable "what kind of activity is this" cue -- Train-mode activity cards get the Workout accent, Nutrition-mode ones the Nutrition accent. Most cards have none; reach for it on a card that represents one real, completed piece of activity (a workout, a logged meal), not as general decoration.
...
- **Feed composition** (`screens/FeedScreen.tsx`, replacing the old Dashboard). The app's landing screen: a personal activity feed -- there is no following/social graph, so this is "what did I actually do", never anyone else's data. One data source per item kind (`feed/feedQueries.ts`): the user's own completed workouts (paginated, oldest reachable via **Load More**) and this week's logged foods, merged into a single reverse-chronological stack of `AppCard`s, `widgetGap` apart. Every card carries a `topAccent` band (§10) in its mode's colour -- blue for a workout, green for a food log -- the one deliberate Strava borrowing in this system. A workout card shows its split day/name, muscles trained, and Sets/Volume as `StatBlock`s (Volume in the accent); a food card shows its picture (`FoodImage`, §17's Nutrition section), name, meal + date, and calories as a `StatValue`. Tapping a card opens that workout's detail or Nutrition Today. Loading/error/empty states use the shared `LoadingState`/`ErrorState`/`EmptyState` exactly like any other screen -- nothing here is a special case.
