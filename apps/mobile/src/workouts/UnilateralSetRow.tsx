 ...
 * are never added together anywhere in this component. Reuses SetRow's own
 * input/complete-button styling (setInput/setInputCompleted/
 * setCompleteButton) so a unilateral exercise's sets look like a natural
 * extension of a bilateral one, not a different visual language.
 */
export function UnilateralSetRow({
  ...
  return (
    <View testID={testID} style={styles.unilateralSetRow}>
      <Text style={styles.unilateralSetIndex}>{setIndex}</Text>

      <View style={styles.unilateralSideRows}>
        <View style={styles.unilateralSideRow}>
          <Text style={styles.unilateralSideLabel}>L</Text>
          <TextInput
            testID={testID ? `${testID}-left-weight` : undefined}
            style={[styles.setInput, completed && styles.setInputCompleted]}
            keyboardType="decimal-pad"
            value={left.weight}
            onChangeText={(text) => onChangeWeight('left', text)}
            editable={!completed}
            placeholder="0"
            placeholderTextColor={colors.textMuted}
          />
          <TextInput
            testID={testID ? `${testID}-left-reps` : undefined}
            style={[styles.setInput, completed && styles.setInputCompleted]}
            keyboardType="number-pad"
            value={left.reps}
            onChangeText={(text) => onChangeReps('left', text)}
            editable={!completed}
            placeholder="0"
            placeholderTextColor={colors.textMuted}
          />
        ...
        <View style={styles.unilateralSideRow}>
          <Text style={styles.unilateralSideLabel}>R</Text>
          <TextInput
            testID={testID ? `${testID}-right-weight` : undefined}
            style={[styles.setInput, completed && styles.setInputCompleted]}
            keyboardType="decimal-pad"
            value={right.weight}
            onChangeText={(text) => onChangeWeight('right', text)}
            editable={!completed}
            placeholder="0"
            placeholderTextColor={colors.textMuted}
          />
          <TextInput
            testID={testID ? `${testID}-right-reps` : undefined}
            style={[styles.setInput, completed && styles.setInputCompleted]}
            keyboardType="number-pad"
            value={right.reps}
            onChangeText={(text) => onChangeReps('right', text)}
            editable={!completed}
            placeholder="0"
            placeholderTextColor={colors.textMuted}
          />
