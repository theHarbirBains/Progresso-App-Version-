...
export function WorkoutColorScreen({ navigation }: Props) {
  ...
  const [currentColor, setCurrentColor] = useState(DEFAULT_WORKOUT_COLOR);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (mounted) setCurrentColor(profile.workoutAccentColor ?? DEFAULT_WORKOUT_COLOR);
      } finally {
  ...
  async function handleSave(hex: string) {
    ...
    setSaving(true);
    try {
      await updateMyProfile(accessToken, { workoutAccentColor: hex });
      navigation.goBack();
    } catch (err) {
